...
public class TablapapeleriaJpaController implements Serializable {
    ...
    public void edit(Tablapapeleria tablapapeleria)
        ...
        try {
            ...
            if (msg ==
                ...
                if (findTablapapeleria(id) ==
                        null) {
                    throw new NonexistentEntityException("The tablapapeleria with id " +
                            id +
                            " no longer exists.");
                }
            }
    ...
    public void destroy(Integer id)
        ...
        try {
            ...
            try {
                tablapapeleria
                        = em.getReference(Tablapapeleria.class,
                        id);
                tablapapeleria.getId();
    ...
    public List<Tablapapeleria> findTablapapeleriaEntities() {
        return findTablapapeleriaEntities(true,
                -1,
                -1);
    }

    public List<Tablapapeleria> findTablapapeleriaEntities(int maxResults,
            int firstResult) {
        return findTablapapeleriaEntities(false,
                maxResults,
                firstResult);
    }

    private List<Tablapapeleria> findTablapapeleriaEntities(boolean all,
            int maxResults,
            int firstResult) {
        EntityManager em
                = getEntityManager();
    ...
    public Tablapapeleria findTablapapeleria(Integer id) {
        EntityManager em
                = getEntityManager();
        try {
            return em.find(Tablapapeleria.class,
                    id);
        } finally {
            em.close();
        }
    ...
    public int getTablapapeleriaCount() {
        ...
        try {
            CriteriaQuery cq
                    = em.getCriteriaBuilder().
                    createQuery();
            Root<Tablapapeleria> rt
                    = cq.from(Tablapapeleria.class);
